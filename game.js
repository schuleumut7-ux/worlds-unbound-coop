...
function update(dt){if(mode!=='game')return;attackCd=Math.max(0,attackCd-dt);abilityCd=Math.max(0,abilityCd-dt);if(hero.downed){
  ...
  if(mobileEnabled&&joy.on){dx=joy.x/45;dy=joy.y/45}
...
let dx=(keys.has('d')||keys.has('arrowright')?1:0)-(keys.has('a')||keys.has('arrowleft')?1:0),dy=(keys.has('s')||keys.has('arrowdown')?1:0)-(keys.has('w')||keys.has('arrowup')?1:0);if(mobileEnabled&&joy.on){dx=joy.x/45;dy=joy.y/45}const l=Math.hypot(dx,dy)||1;if(dx||dy){const moveSpeed=hero.speed*(hero.carrying?0.9:1);hero.x=Math.max(40,Math.min(worldSize.w-40,hero.x+dx/l*moveSpeed*dt));hero.y=Math.max(40,Math.min(worldSize.h-40,hero.y+dy/l*moveSpeed*dt));hero.stamina=Math.max(0,hero.stamina-dt*4)}else hero.stamina=Math.min(100,hero.stamina+dt*10);
for(const m of mobs){m.a+=dt;if(m.hit>0)m.hit-=dt;const d=Math.hypot(hero.x-m.x,hero.y-m.y);if(hero.downed)continue;if(d<300){m.x+=(hero.x-m.x)/Math.max(d,1)*m.speed*dt;m.y+=(hero.y-m.y)/Math.max(d,1)*m.speed*dt}if(d<30&&Math.random()<dt*.7)damagePlayer(5+world*2)}
